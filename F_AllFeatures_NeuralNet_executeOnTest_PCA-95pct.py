...
if __name__ == '__main__':
    ...
    test_df = pd.read_csv(Path('feature-dataframes/PatLvDiv_TEST-AllFeats_1387-Features_1503-images.csv'), index_col=0)
    print('Done Read Test Dataframe!')

    print('Preparing Data...')

    x_train, y_train, x_test, y_test = prepareData(train_df=train_df, valid_df=test_df)
    ...
    for modelFile in os.listdir(modelsPaths):
        ...
        if '_PCA95pct.h5' not in modelFile:
            continue
    ...
    result_df.to_csv(Path('results/NeuralNet_AllFeatures_TestPerformance_PCA-95pct.csv'))

    y_preds = np.array(y_preds)
    with open(Path('results/y_preds_PCA-95pct.txt'), 'w') as f:
