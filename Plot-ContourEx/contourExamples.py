...
def plotContourSignature(grayScaleImage, sizeVector=320):
    ...
    plt.subplot(222)
    plt.imshow(cv2.bitwise_not(contourPoints),cmap='gray')
    plt.title('LYMPHOCYTE CONTOUR', fontname='Times New Roman', fontsize=18)
    ...
    with plt.style.context(('ggplot')):
        plt.subplot(223)
        plt.plot(signature)
        plt.title(f'CENTROID DISTANCE FUNCTION  ({len(signature)} samples)', fontname='Times New Roman', fontsize=18)
    ...
    with plt.style.context(('ggplot')):
        ...
        plt.title('FOURIER DESCRIPTORS', fontname='Times New Roman', fontsize=18)
