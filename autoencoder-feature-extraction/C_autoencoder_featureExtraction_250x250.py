...
def testAutoEncoder(enc, dec, imgPath):
    ...
    pImg= processImg(imgPath, subtractMean=True, divideStdDev=False, colorScheme='rgb', imgSize=(250,250))
    #Encode
...
def extractAutoEncoderFeatures():
    ...
    for mainDirectory in [(augm_patLvDiv_train, 'AugmPatLvDiv_TRAIN'), (augm_patLvDiv_valid, 'AugmPatLvDiv_VALIDATION'), (patLvDiv_test, 'PatLvDiv_TEST')]:
        ...
        df = df[cols]
        df.to_csv(f'feature-dataframes/450x450-images_{mainDirectory[1]}_{df.shape[1]-1}-Reduced-PCA-Autoencoder-Features_{df.shape[0]}-images.csv')


def createDataframes():
    #Create TRAIN dataframe
    def createTrainDataframe():
        df = Path('feature-dataframes/450x450-images_AugmPatLvDiv_TRAIN_125-Reduced-PCA-Autoencoder-Features_40000-images.csv')
        df = pd.read_csv(df, index_col=0)
        ...
        trainDF.to_csv(f'feature-dataframes/450x450-images_AugmPatLvDiv_TRAIN_{trainDF.shape[1]-1}-Features_{trainDF.shape[0]}-images_450x450.csv')

    #Create VALIDATION dataframe
    def createValidDataframe():
        df = Path('feature-dataframes/450x450-images_AugmPatLvDiv_VALIDATION_125-Reduced-PCA-Autoencoder-Features_10000-images.csv')
        df = pd.read_csv(df, index_col=0)
        ...
        validDF.to_csv(f'feature-dataframes/450x450-images_AugmPatLvDiv_VALIDATION_{validDF.shape[1]-1}-Features_{validDF.shape[0]}-images_450x450.csv')

    #Create TEST dataframe
    def createTestDataframe():
        df = Path('feature-dataframes/450x450-images_PatLvDiv_TEST_125-Reduced-PCA-Autoencoder-Features_1503-images.csv')
        df = pd.read_csv(df, index_col=0)
        testDF = pd.DataFrame()
        for n, row in enumerate(df.iterrows()):
        ...
        cols = ['cellType(ALL=1, HEM=-1)'] + cols
        testDF = testDF[cols]
        testDF.to_csv(f'feature-dataframes/450x450-images_AugmPatLvDiv_TEST_{testDF.shape[1]-1}-Features_{testDF.shape[0]}-images_450x450.csv')
...
if __name__ == '__main__':
    decoder = Path('Trained-AutoEncoder/250x250_06871986_Loss-37.702_Epoch-083-decoder.h5')
    encoder = Path('Trained-AutoEncoder/250x250_06871986_Loss-37.702_Epoch-083-encoder.h5')
    encoder = load_model(str(encoder)) #Load encoder
